...
void readFile(char* filename,vector<vector<int> >& clauses, map<int,int>& assignments)
{
...
int isValid(vector<vector<int>> clauses, map<int,int> curr_assignments)
{
...
void outputResult(map<int, int> curr_assignments, ofstream& ofile)
{
	for(map<int,int>::iterator it = curr_assignments.begin();
...
bool recursiveHelper(vector<vector<int>> clauses, map<int, int>& curr_assignments, int curr_var)
{
	...
	{
		map<int, int>::iterator it = curr_assignments.find(curr_var);
...
int main(int argc, char* argv[])
{
	...
	map<int, int> curr_assignments; //-1, 0, 1
